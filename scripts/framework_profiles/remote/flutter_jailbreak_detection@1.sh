#!/bin/bash
# =============================================
#   Profile: flutter_jailbreak_detection (remote, renamed)
#   Version: 1.x (ObjC bridge + Swift)
# =============================================

PROFILE_NAME="flutter_jailbreak_detection"
PROFILE_VERSION="1"
PROFILE_STATUS="draft"

PROFILE_PROTECTED=(
    "ReadInitializingPlugin"
    "SwiftReadInitializingPlugin"
    ...
    for src_dir in "${source_dirs[@]}"; do
        ...
        if [[ "$level" == "L3" ]]; then
            while IFS= read -r f; do
                [[ -f "$f" ]] || continue
                file_has_preprocessor_blocks "$f" && continue
                bt_inject_dead_branches "$f" 6
            done < <(find "$src_dir" -name "*.m" -type f ! -name "${INJECT_PREFIX}*" 2>/dev/null)
            while IFS= read -r f; do
                [[ -f "$f" ]] && bt_inject_swift_dead_branches "$f" 6
            done < <(find "$src_dir" -name "*.swift" -type f ! -name "Package.swift" 2>/dev/null)
        fi
    done
}
