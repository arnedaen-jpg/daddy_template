...
核心文件（--callstack 针对的文件，每个项目不同）:
  dq:     module_entry.dart, main.dart, http_manager.dart, global_logic.dart, xx_domain_manager.dart 等（斗球/直播基础设施层）
  lgt:    module_entry.dart, app_service.dart, api_*.dart 全量, app_routes.dart 等（聊个天/IM 服务层 + API + 路由）

示例:
  dart run bin/obfuscate.dart -p dq --string              # dq 项目字符串混淆
  dart run bin/obfuscate.dart -p dq --callstack           # dq 项目调用栈混淆
  dart run bin/obfuscate.dart -p dq --bloat               # dq 项目文件膨胀（4.3a）
  dart run bin/obfuscate.dart -p dq --noise               # 业务噪音注入
  dart run bin/obfuscate.dart -p dq --all                 # 所有混淆
