...
class Obfuscator {
  ...
  Future<void> run() async {
    ...
    _logger.info('');
    _logger.info('混淆功能:');
    _logger.info('  字符串混淆: ${config.enableString ? "✓ 启用 (${config.stringMethod})" : "✗ 禁用"}');
    _logger.info('  调用栈混淆: ${config.enableCallstack ? "✓ 启用 (深度: ${config.callStackDepth})" : "✗ 禁用"}');
    ...
    _logger.info('  2. 运行 flutter build ipa --release --obfuscate --split-debug-info=build/symbols');
  }
  ...
  Future<void> _initAnalysisContext() async {
    ...
    final absolutePath = path.absolute(config.targetDir);
  ...
  Future<List<String>> _collectDartFiles() async {
    ...
    await for (final entity in dir.list(recursive: true, followLinks: false)) {
      if (entity is File && entity.path.endsWith('.dart')) {
        ...
        final shouldExclude = config.excludeFilePatterns.any(
