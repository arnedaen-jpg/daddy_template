...
class StringObfuscator {
  ...
  Future<void> process(List<String> files, AnalysisContextCollection collection) async {
    ...
    for (final file in files) {
      ...
      try {
        ...
        if (visitor.sensitiveStrings.isEmpty && visitor.skippedStrings.isEmpty) {
          continue;
        }
        
        // 写入报告 —— 按文件分组
        if (visitor.sensitiveStrings.isNotEmpty || visitor.skippedStrings.isNotEmpty) {
          report.writeln('\n## $relPath');
          report.writeln('  混淆: ${visitor.sensitiveStrings.length}  跳过: ${visitor.skippedStrings.length}');
        ...
        if (newContent != content) {
          ...
          totalFiles++;
          totalStrings += visitor.sensitiveStrings.length;
          logger.debug('混淆 ${visitor.sensitiveStrings.length} 个字符串: ${path.basename(file)}');
        }
  ...
  void _writeReport(
    ...
    final reportFile = File('build/string_obfuscation_report_$projectName.txt');
...
class _StringInfo {
  ...
  _StringInfo(this.value, this.offset, this.hasDoubleQuote, this.lexemeLength, {this.isRawString = false});
}
...
class _StringCollector extends RecursiveAstVisitor<void> {
  ...
  void visitClassDeclaration(ClassDeclaration node) {
    ...
    _classHasConstConstructor = node.members.any((m) => 
  ...
  void visitSimpleStringLiteral(SimpleStringLiteral node) {
    ...
    sensitiveStrings.add(_StringInfo(
      value, node.offset, hasDoubleQuote, lexeme.length,
  ...
  bool _isGenericString(String value) {
    ...
    const dataKeys = {
      'id', 'type', 'name', 'data', 'code', 'msg', 'message',
      'status', 'result', 'error', 'success', 'list', 'info',
      'url', 'key', 'value', 'title', 'text', 'image', 'icon',
      'time', 'date', 'page', 'size', 'total', 'count', 'index',
      'true', 'false', 'null', 'yes', 'no', 'ok', 'OK',
    };
